...
mod tests {
    ...
    #[test]
    fn leap_year_rules() {
        assert!(is_leap(2000)); // 400 의 배수
        assert!(!is_leap(1900)); // 100 의 배수 but not 400
        assert!(is_leap(2024)); // 4 의 배수
        assert!(!is_leap(2023));
        assert!(is_leap(2400));
        assert!(!is_leap(2100));
    }
    ...
    fn epoch_known_dates() {
        ...
        assert_eq!(epoch_to_ymdhms(946684800), (2000, 1, 1, 0, 0, 0));
        ...
        assert_eq!(epoch_to_ymdhms(1704067199), (2023, 12, 31, 23, 59, 59));
    }

    #[test]
    fn epoch_handles_leap_day() {
        // 2024-02-29 → 2024-03-01 경계
        let feb29_noon = 1709208000; // 2024-02-29 12:00:00
        let (_, mo, d, _, _, _) = epoch_to_ymdhms(feb29_noon);
        assert_eq!((mo, d), (2, 29));
        let mar01_noon = 1709294400; // 2024-03-01 12:00:00
        let (_, mo, d, _, _, _) = epoch_to_ymdhms(mar01_noon);
        assert_eq!((mo, d), (3, 1));
    }

    #[test]
    fn days_in_months_february() {
        assert_eq!(days_in_months(2024)[1], 29); // 윤년
        assert_eq!(days_in_months(2023)[1], 28);
        assert_eq!(days_in_months(2000)[1], 29);
        assert_eq!(days_in_months(1900)[1], 28);
    }
    ...
    fn fresh_tmp(label: &str) -> std::path::PathBuf {
        let ns = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let p = std::env::temp_dir().join(format!("og-backup-{label}-{ns}"));
        std::fs::create_dir_all(&p).unwrap();
        p
    }

    #[tokio::test]
    async fn backup_once_creates_snapshot_file() {
        use sqlx::sqlite::SqlitePoolOptions;

        let dir = fresh_tmp("once");
        let db_url = format!(
            "sqlite:{}?mode=rwc",
        ...
        let target = backup_once(&pool, dir.to_str().unwrap()).await.unwrap();
        assert!(target.exists(), "backup file must exist");
        assert!(
            target.file_name().unwrap().to_string_lossy().starts_with("guild.db."),
    ...
    fn prune_keeps_only_most_recent_n() {
        let dir = fresh_tmp("prune");
        let backups = dir.join("backups");
        std::fs::create_dir_all(&backups).unwrap();

        // BACKUP_KEEP (168) 보다 많이 생성 → 정확히 168 개 남는지 검증
        for i in 0..170 {
            let name = format!("guild.db.2026{i:04}-000000");
            std::fs::write(backups.join(&name), b"").unwrap();
        }
        // 무관 파일은 prune 대상 X
        std::fs::write(backups.join("README.txt"), b"keep me").unwrap();

        prune_old_backups(dir.to_str().unwrap()).unwrap();

        let remaining: Vec<_> = std::fs::read_dir(&backups)
            .unwrap()
            .filter_map(|e| e.ok())
            .map(|e| e.file_name().to_string_lossy().to_string())
            .filter(|n| n.starts_with("guild.db."))
            .collect();
        assert_eq!(remaining.len(), BACKUP_KEEP);

        let mut sorted = remaining.clone();
        sorted.sort();
        // 가장 오래된 것 (0, 1) 은 삭제 — 남은 oldest 는 index 2
        assert!(sorted[0].contains("guild.db.20260002"));
        assert!(sorted.last().unwrap().contains("guild.db.20260169"));

        assert!(backups.join("README.txt").exists(), "non-backup file untouched");

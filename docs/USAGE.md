...
환경변수라서 **파이프 건너 손자까지** 따라갑니다 — `claude -p | openguild quest comment add …`
의 `openguild` 도 물려받습니다. 반대로 **체인이 끊기면 막을 것도 없습니다.** `env -i` 나
`env -u OPENGUILD_PLUGIN_CHAIN` 으로 환경을 비우고 부르거나, `nohup … &` 로 떼어 던져 놓고
한참 뒤에 다른 경로로 길드를 바꾸면 그 변경은 "사람이 한 것" 으로 보입니다.

**이벤트는 쓰기를 한 그 프로세스가 만듭니다.** 파일을 지켜보다가 나오는 것이 아닙니다. 훅이
부른 CLI 가 댓글을 달면 `comment.added` 도 그 CLI 안에서 만들어지고 거기 꽂힌 플러그인이
받습니다 — 같은 길드를 열어 둔 앱은 그 댓글에 대해 이벤트를 내지 않습니다(화면에는 다시
읽을 때 보입니다).

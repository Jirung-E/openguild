...
- **commit 은 원칙적으로 사용자가 명시적으로 요청할 때만 — 단 하나의 예외.**
  - **예외(2026-08-08 확정)**: 퀘스트 **구현을 시작**하면(착수 당시 상태가 Open
    이든 On Hold 든 무관) `In Progress` 로 옮기고, **구현이 끝나** `Testing` 으로
    옮기는 그 순간 **자동으로 커밋한다** (묻지 않음). 이 예외는 딱 그 한 지점
    (Testing 전환 직후)에만 적용 — `Testing → Done` 전환, 여러 퀘스트를 모아
    처리하는 경우, chore 성 변경 등 **그 외의 모든 커밋은 여전히 사용자에게
    먼저 물어본다.**

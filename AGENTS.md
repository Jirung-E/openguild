...
### `just test` 는 **커밋 직전**에 돌린다

앞에서 한 번 통과했다는 것은 **커밋되는 상태의 증거가 아니다.** 통과시킨 뒤에
무엇이든 손댔다면 그 상태는 검증된 적이 없다.

실제로 그랬다(DEV-383): 되돌림 시험을 하느라 파일을 고쳤다 되돌리는 사이에
아직 커밋 안 된 변경이 날아갔고, `just test` 는 그 **전에** 돌렸으므로 통과했다.
프런트 타입 검사가 깨진 채로 develop 에 머지됐고, 다음 작업의 첫 `svelte-check`
가 잡을 때까지 몰랐다.

### 되돌림 시험의 원복은 **백업 파일에서** 한다

고친 것이 정말 효과가 있는지 보려면 되돌려서 시험이 깨지는지 봐야 한다. 그때
`git checkout -- <파일>` 로 되돌리면 **아직 커밋 안 된 작업까지 같이 지운다.**

```sh
cp 파일 /tmp/백업     # 되돌리기 전에
...                   # 되돌려서 시험이 무는지 확인
cp /tmp/백업 파일     # 원복은 여기서
```

...
- **`just test` 는 이 기계의 OS 만 본다.** CI 는 리눅스에서도 `cargo clippy -D warnings`
  를 돌린다. 그래서 `#[cfg(target_os = "…")]` 로 감싼 코드를 새로 쓰면, 그 OS 가 아닌
  빌드에서 그 함수가 **아무 데서도 안 쓰여** `dead_code` 가 나고 CI 만 빨개진다(맥에서
  통과한 것을 푸시한 뒤 리눅스에서 걸렸다 — BUG-301).
  쓰는 쪽과 **같은 조건**으로 경고를 함께 꺼 둔다:

  ```rust
  #[cfg(target_os = "macos")]           // 쓰는 쪽
  #[cfg_attr(not(target_os = "macos"), allow(dead_code))]   // 정의 쪽
  ```

  정의째 `#[cfg]` 로 잘라 내는 것보다 낫다 — 어디서나 컴파일되어야 그 규칙을 고정한
  시험이 모든 OS 에서 돈다. 확인은 두 조건을 **같이 뒤집어** 한 번 돌려 보면 된다.

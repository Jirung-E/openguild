...
updated_at = "2026-09-17T09:37:03+09:00"
+++
...
| 11 | **DEV-306** 백업 스냅샷 1개 (시드 중 자동 백업도 함께 생김) | 관리 > 백업 목록/복원 UI |
| 12 | **BUG-290/291, DEV-392/397/398/399** 한글 이름 태그 정의(색·설명) + 퀘스트에 부착 / 도서관 2단 폴더(`아키텍처/결정`) + 빈 폴더(`보관`) + 그 안 문서 / 표시 이름에 확장자가 없는 긴 PDF 첨부 / `examples/plugins/*` 를 `.guild/plugins` 로 복사(동의 대기 상태) | 태그 페이지 색 편집 / 폴더 트리·폴더 끌어 옮기기 / 첨부 종류 배지(PDF)·가운데 줄임 / 관리 > 플러그인 목록·설정값 입력 |
...
- 2026-09-17: 단계 12 추가 — 태그 정의(한글 이름·색·설명), 도서관 폴더 계층,
  긴 첨부 이름, 예제 플러그인 복사. 단계 11 의 백업 화면 위치를 관리 > 백업으로
  정정(DEV-393 에서 설정 → 관리로 옮김). 한계 절의 "태그 CLI 없음" 제거
  (`openguild tag`·`quest tag` 가 있다). 예제 플러그인은 복사만 한다 — 동의는
  기계마다 따로라 시드가 대신 허용하지 않는다.

...
| `restore --at <미래 ISO>` | 최신(항상) | 전체 | truncate | 현재(최신) 상태 복구 | 상태는 무손실이나 journal 히스토리 사라짐 |
...
- 복원 의도별 요약: **최신으로 복구(손상 복구)** = `restore --at <미래>` (= 최신 스냅샷 + journal 전체 replay). **특정 백업으로 되돌리기** = `restore --to <ts>`(가역).

...
; DEV-034: setup.exe / uninstall.exe 의 .exe 리소스 아이콘.
; Tauri 가 INSTALLERICON / UNINSTALLERICON 변수는 채워주지만 NSIS Icon /
; UninstallIcon 명령 자체는 안 함 — 명시. 안 하면 NSIS 기본 (다운로드 화살표).
Icon "${INSTALLERICON}"
...
; 5.5. Components page (DEV-034): select GUI / CLI / Server / PATH.
!define MUI_PAGE_CUSTOMFUNCTION_PRE SkipIfPassive
...
; DEV-034: Section -Core (hidden, always installed).
; - / SetOutPath
; - / Run-check
; - / Uninstaller + Add-Remove Programs registry entries
; - / Resources (always with GUI semantics — Tauri injects raw paths).
; - / External binaries (tauri externalBin — empty in our config).
Section "-Core" SecCore
  ...
  ; Copy external binaries (tauri externalBin — none in our config)
  {{#each binaries}}
...
; DEV-034: Section "Desktop App (GUI)" — default checked.
;   GUI exe + file associations + deep links + start menu / desktop shortcuts.
Section "Desktop App (GUI)" SecGui
  SetOutPath $INSTDIR

  ; Copy main executable (GUI)
  File "${MAINBINARYSRCPATH}"

  ; Create file associations (only meaningful with GUI installed)
...
; DEV-034: Section "Command-Line Tool (openguild)" — default checked.
; Source path is relative from makensis work dir (target\release\nsis\)
; to target\release\openguild.exe.
...
; Common post-install (always runs after sections complete)
...
Section Uninstall
  ...
  ; DEV-034: Delete CLI / Server binaries (always attempt — no-op if not present)
  Delete "$INSTDIR\openguild.exe"
  Delete "$INSTDIR\openguild-server.exe"

...
describe('humanizeUpdaterError', () => {
	it('404 → endpoint / release 안내', () => {
		expect(humanizeUpdaterError('Request failed with status 404')).toMatch(
			/릴리즈가 아직 없거나/
		);
	});
	it('not found 도 같은 안내', () => {
		expect(humanizeUpdaterError('endpoint not found')).toMatch(
			/릴리즈가 아직 없거나/
		);
	});
	...
	it('signature 키워드 → 서명 검증 실패', () => {
		expect(humanizeUpdaterError('Signature verification failed')).toMatch(
			/서명 검증/
		);
		expect(humanizeUpdaterError('invalid pubkey')).toMatch(/서명 검증/);
	});
	it('dev / debug → 개발 빌드 안내', () => {
		expect(humanizeUpdaterError('dev mode update not supported')).toMatch(
			/개발 빌드/
		);
	});

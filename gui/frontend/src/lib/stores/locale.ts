...
const DICT: Record<string, { ko: string; en: string }> = {
	...
	'admin.noAuthWarn': { ko: '⚠ 인증 없음 — MVP 단계. 멀티유저로 확장 시 보호 필요.', en: '⚠ No authentication — MVP stage. Protection needed for multi-user use.' },
	...
	'admin.noBackups': { ko: '백업 없음. "+ 새 백업" 을 눌러 첫 백업을 생성하세요.', en: 'No backups. Press "+ New backup" to create the first one.' },
	...
	'admin.autoBackupHint': { ko: '자동 백업: 매 mutation 후 정책 검사 (ops 50 회 OR 24 시간 도달 시).', en: 'Auto-backup: policy check after every mutation (at 50 ops OR 24 hours).' },
	...
	'admin.driftEmpty': { ko: '파일 vs index.db 일치성 검사. 외부 편집 / git pull 후 활용.', en: 'Checks file vs index.db consistency. Use after external edits / git pull.' },
	'admin.driftOk': { ko: '✓ drift 없음', en: '✓ No drift' },
	'admin.missingInIndex': { ko: '파일은 있는데 index 에 없음 (', en: 'File exists but missing from index (' },
	...
	'admin.reindexHint': { ko: 'Reindex 버튼으로 캐시를 파일 기준으로 재구축할 수 있습니다.', en: 'Use the Reindex button to rebuild the cache from files.' },
	...
	'admin.journalEmpty': { ko: '저널 비어 있음 (snapshot 직후 또는 mutation 없음)', en: 'Journal empty (right after a snapshot or no mutations)' },
	...
	'admin.restoreDonePost': { ko: ' — 파일 복구 + 재색인 완료. 새로고침합니다.', en: ' — files restored + reindexed. Reloading.' },
	...
	'admin.driftOkMsg': { ko: 'drift 없음 — 파일과 캐시 일치', en: 'No drift — files and cache match' },
	'admin.driftFoundPost': { ko: ' 항목 drift 발견 — 아래 보고서 확인', en: ' item(s) of drift found — see report below' },
	'admin.driftCheckFailedPre': { ko: 'drift 검사 실패: ', en: 'Drift check failed: ' },
	'admin.reindexConfirm': { ko: '파일들로부터 index.db 를 재구축합니다. 계속할까요?', en: 'Rebuild index.db from files. Continue?' },
	'admin.reindexSkippedPre': { ko: 'reindex — 비정상 파일 ', en: 'reindex — skipped ' },
	'admin.reindexSkippedPost': { ko: '개 건너뜀. 새로고침 후 상세 표시.', en: ' problem file(s). Details after reload.' },
	'admin.reindexDone': { ko: 'reindex 완료 — 데이터 새로고침', en: 'Reindex complete — reloading data' },
	...
	'admin.vacuumNoSpace': { ko: 'VACUUM 완료 — 회수 공간 없음 (이미 dense)', en: 'VACUUM complete — nothing to reclaim (already dense)' },
	'admin.vacuumFailedPre': { ko: 'VACUUM 실패: ', en: 'VACUUM failed: ' },
	'admin.journalEmptyMsg': { ko: 'journal.db 비어 있음 (snapshot 직후 또는 mutation 없음)', en: 'journal.db empty (right after a snapshot or no mutations)' },
	'admin.journalLoadFailedPre': { ko: 'journal 조회 실패: ', en: 'Failed to load journal: ' },
	'admin.problemFilesHint': { ko: '아래 파일은 파싱 실패 / 정의되지 않은 status 로 reindex·동기화에서 건너뛰어집니다. 파일을 고치거나 status 를 정의한 뒤 Reindex 하세요.', en: 'The files below are skipped during reindex/sync due to parse failure / undefined status. Fix the file or define the status, then Reindex.' },
	'admin.maintEmptyHint': { ko: 'VACUUM: index.db 의 dead row 공간 회수. 저널: journal.db 의 최근 변경 op (AOF).', en: 'VACUUM: reclaims dead-row space in index.db. Journal: recent change ops in journal.db (AOF).' },
	...
	'admin.restoreConfirmPost': { ko: '현재 상태가 덮어써집니다 (직전 .pre-restore.db 로 자동 백업됨).', en: 'The current state will be overwritten (auto-backed up to .pre-restore.db first).' },
	'admin.deleteConfirmPre': { ko: '"', en: 'Delete backup "' },
	'admin.deleteConfirmPost': { ko: '" 백업을 삭제할까요?\n\n이 백업 파일이 영구 삭제됩니다 (되돌릴 수 없음).', en: '"?\n\nThis backup file will be permanently deleted (cannot be undone).' },
	...
	'adminTypes.renameCascadeHint': { ko: 'prefix 자체 rename 은 quest slug cascade 라 지원 안 함.', en: 'Renaming prefix itself requires a quest-slug cascade — not supported here.' },
	'adminTypes.newTypeTitle': { ko: '새 quest type', en: 'New quest type' },
	'adminTypes.prefixPlaceholder': { ko: 'DEV / BUG / REQ 같은 1~6자', en: '1–6 chars like DEV / BUG / REQ' },
	...
	'adminTypes.renamePrefixTitle': { ko: 'Type prefix 변경 (cascade)', en: 'Change type prefix (cascade)' },
	'adminTypes.renameConfirmMid': { ko: ' 로 이름 변경.\n\n', en: '.\n\n' },
	'adminTypes.renameConfirmCount1': { ko: '이 type 의 모든 quest (', en: 'All quests of this type (' },
	'adminTypes.renameConfirmCount2': { ko: '개) 의 slug 가 cascade 됩니다 (파일명 / frontmatter / DB history).\n\n', en: ') will have their slug cascaded (filename / frontmatter / DB history).\n\n' },
	'adminTypes.renameConfirmMention1': { ko: "다른 quest 본문 안의 '", en: "Mentions of '" },
	'adminTypes.renameConfirmMention2': { ko: "-NNN' mention 은 자동 갱신되지 않습니다 — 직접 검색/수정 필요.\n\n계속할까요?", en: "-NNN' in other quest bodies are not updated automatically — search and edit manually.\n\nContinue?" },
	...
	'adminStatuses.doneColTitle': { ko: "캠페인 진행도 계산 시 '완료' 로 카운트되는 status", en: "Status counted as 'Done' for campaign progress" },
	'adminStatuses.doneCol': { ko: '완료', en: 'Done' },
	'adminStatuses.slugTitle': { ko: "소문자/숫자/'_' 만, 최대 32자", en: "Lowercase/digits/'_' only, max 32 chars" },
	'adminStatuses.nameEnTitle': { ko: "영문자로 시작 + 영문 / 숫자 / 공백 / '-' / '_' 만, 최대 32자", en: "Starts with a letter; letters/digits/space/'-'/'_' only, max 32 chars" },
	'adminStatuses.countsAsDoneTitle': { ko: "캠페인 진행도 계산 시 '완료' 로 카운트", en: "Counted as 'Done' for campaign progress" },
	'adminStatuses.countsAsDoneMark': { ko: '이 status 는 완료로 카운트', en: 'This status counts as done' },
	'adminStatuses.slugFrozenHint': { ko: 'slug 는 frozen — history / 파일 frontmatter 가 참조하므로 rename 안 됨.', en: 'slug is frozen — history / file frontmatter reference it, so renaming is not allowed.' },
	'adminStatuses.newStatusTitle': { ko: '새 quest status', en: 'New quest status' },
	'adminStatuses.nameEnPlaceholder': { ko: 'Blocked / In Review 등', en: 'e.g. Blocked / In Review' },
	'adminStatuses.nameKoPlaceholder': { ko: '(선택) 막힘 / 리뷰 중 등', en: '(optional) localized name' },
	'adminStatuses.newStatusNote': { ko: '새 status 는 목록 맨 뒤에 추가됩니다.', en: 'New statuses are added at the end of the list.' },
	...
	'adminStatuses.deleteMsg3': { ko: ' 내 파일도 함께 제거됩니다.', en: ' on disk will also be removed.' },
	'adminStatuses.renameSlugTitle': { ko: 'Status slug 변경 (cascade)', en: 'Change status slug (cascade)' },
	'adminStatuses.renameSlugConfirm1': { ko: ' 로 slug 변경.\n\n', en: '.\n\n' },
	'adminStatuses.renameSlugConfirm2': { ko: '개 quest 의 frontmatter status + history 의 old/new value + statuses 파일명 모두 cascade.\n\n계속할까요?', en: " quests' frontmatter status + history old/new value + statuses filename all cascade.\n\nContinue?" },
	'adminStatuses.listLoadFailedPre': { ko: 'status 목록 조회 실패: ', en: 'Failed to load status list: ' },
	...
	'adminTags.pathHintPost': { ko: ' 의 색 / 설명 — quest·도서관·규칙이 공유하는 태그', en: ' color / description — tags shared by quests, library, and rules' },
	'adminTags.empty': { ko: '정의된 tag 없음.', en: 'No tag definitions.' },
	'adminTags.slugTitle': { ko: "소문자 / 숫자 / '_' 만, 최대 32자", en: "Lowercase/digits/'_' only, max 32 chars" },
	...
	'adminTags.descPurposePlaceholder': { ko: '(선택) 이 tag 의 용도', en: '(optional) purpose of this tag' },
	...
	'adminTags.deleteMsg2': { ko: '기존 quest 의 tag 사용은 그대로 (fallback 색).', en: 'Existing quest tag usages are kept (fallback color).' },
	'adminTags.introTail': { ko: ' registry. 정의가 없는 tag 도 사용 가능(UI 기본 색으로 표시); 여기서는 색/설명만 미리 정의해둔다.', en: ' registry. Tags without a definition can still be used (shown with a default UI color); here you predefine color/description.' },
	'adminTags.listLoadFailedPre': { ko: 'tag 정의 조회 실패: ', en: 'Failed to load tag definitions: ' },
	...
	'adminTags.slugPattern': { ko: 'slug 는 소문자/숫자/_ 만 (최대 32자).', en: 'slug must be lowercase/digits/_ only (max 32 chars).' },
	...
	'adminTags.deletedDefPost': { ko: "' 정의 삭제됨 (기존 사용처의 태그는 보존)", en: "' definition deleted (existing usages preserved)" },
	...
	'rules.emptyList': { ko: '규칙 없음. "+ 신규" 로 만들기.', en: 'No rules. Use "+ New" to create one.' },
	'rules.emptyFiltered': { ko: '태그 필터에 맞는 규칙 없음.', en: 'No rules match the tag filter.' },
	...
	'rules.bodyLabel': { ko: '본문 (Markdown) — 첨부는 드래그&드랍 / Ctrl+V', en: 'Body (Markdown) — attach via drag & drop / Ctrl+V' },
	...
	'rules.discardEditMsg': { ko: '편집 중인 변경 사항이 있습니다. 버리고 이동할까요?', en: 'You have unsaved edits. Discard and leave?' },
	'rules.discardAndLeave': { ko: '버리고 이동', en: 'Discard and leave' },
	'rules.emptyCreateFirst': { ko: '"+ 신규" 로 첫 규칙을 만드세요.', en: 'Use "+ New" to create your first rule.' },
	...
	'update.installBtn': { ko: '지금 업데이트 (다운로드 + 재시작)', en: 'Update now (download + restart)' },
	...
	'tagPills.newPlaceholder': { ko: '새 태그 (공백 구분으로 여러 개)', en: 'New tag(s), space-separated' },
	'tagPills.newAria': { ko: '새 태그', en: 'New tag' },

	// DEV-205(2차): 퀘스트/캠페인 콤보박스(검색 결과 없음).
	'combobox.questPlaceholder': { ko: '퀘스트 검색 (ID 또는 제목)', en: 'Search quests (ID or title)' },
	'combobox.campaignPlaceholder': { ko: '캠페인 검색 (C-NNN 또는 제목)', en: 'Search campaigns (C-NNN or title)' },
	...
	'library.folderNameRequired': { ko: '폴더 이름을 입력하세요.', en: 'Please enter a folder name.' },
	'library.createFolderFail': { ko: '폴더 생성 실패', en: 'Failed to create folder' },
	'library.deleteFolderFail': { ko: '폴더 삭제 실패 — 비어 있는지 확인하세요', en: 'Failed to delete folder — check that it is empty' },
	...
	'library.newFolderPathPlaceholder': { ko: '새 폴더 경로 (예: 아키텍처/서브)', en: 'New folder path (e.g. architecture/sub)' },
	...
	'library.emptyFolder': { ko: '비어 있음. "+ 신규" 또는 "+ 폴더" 로 만들기.', en: 'Empty. Create with "+ New" or "+ Folder".' },
	'library.emptyRoot': { ko: '문서 없음. "+ 신규" 로 만들기.', en: 'No documents. Create with "+ New".' },
	'library.topLevel': { ko: '(최상위)', en: '(top level)' },
	'library.pickFirstDoc': { ko: '"+ 신규" 로 첫 문서를 만드세요.', en: 'Create your first document with "+ New".' },
	'library.pickDocFromList': { ko: '좌측에서 문서를 선택하세요.', en: 'Select a document from the list.' },
	...
	'library.bodyHint': { ko: '본문 (Markdown) — 첨부는 드래그&드랍 / Ctrl+V', en: 'Body (Markdown) — attach via drag & drop / Ctrl+V' },
	...
	'library.deleteDocMessage': { ko: '문서를 삭제할까요? (번호는 재사용되지 않습니다)', en: 'Delete this document? (the number is never reused)' },
	'library.deleteFolderTitle': { ko: '폴더 삭제', en: 'Delete folder' },
	'library.editingMoveTitle': { ko: '편집중 이동', en: 'Leave while editing' },
	'library.editingMoveMessage': { ko: '편집 중인 변경 사항이 있습니다. 버리고 이동할까요?', en: 'You have unsaved changes. Discard and leave?' },
	...
	'library.folderDeleteTitle': { ko: '폴더 삭제 (비어 있을 때만)', en: 'Delete folder (only if empty)' },

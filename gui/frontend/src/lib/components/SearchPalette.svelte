...
<style>
	...
	.palette {
		...
		/* BUG-252: rem 이라 UI 배율(root font-size)을 따라간다. 예전엔 560px
		   ...
		   vw 상한 62 → 80 (admin 요청). 62vw 는 "넓은 모니터에서 과하게
		   벌어지지 않게" 넣은 값인데, **높은 배율에서도** 걸려서 1400px 창
		   기준 약 1.55배부터 팔레트가 더 안 커졌다(실측: 150% 840px →
		   175% 868px 에서 멈춤). 배율을 올린 사용자는 크게 보려는 것이므로
		   상한을 올린다. 여전히 화면을 다 먹지는 않는다. */
	...
	/* DEV-364: 모양은 global.css 의 `.pill.mono.xs` 가 정본 — 보드 노드의 slug
	   ...
	   - 상자 높이: 나란히 놓이는 `.ptype` 과 **같아야** 한다. 어긋나면 한쪽이
	     잘못된 것처럼 보인다. BUG-244 가 `.ptype` 을 제목 한 줄에 맞춰
	     1.375rem(22px)으로 키우면서 이쪽을 안 따라와 2.4px 어긋나 있었다.
	   - 글자 크기는 일부러 조금 작다 — 식별자는 고정폭이라 같은 크기면 분류
	     라벨보다 넓어 보인다.
	   - 색: 종류/타입 색(`--kind-c`)을 `.pill` 이 읽는 `--c` 로 넘긴다.

	   `.pill.pslug` 로 적는 이유: `--pill-h` 는 `.pill.xs`(0,2,0)도 정하는데
	   `.pslug`(+ Svelte 스코프 클래스)도 0,2,0 이라 **순서로 갈린다**. global.css
	   가 뒤에 오므로 그냥 `.pslug` 로 쓰면 기본값 1.7em 이 이긴다. 실제로 그렇게
	   썼다가 브라우저 실측에서 19.58px 로 안 바뀌는 것을 확인하고 고쳤다. */
	.pill.pslug {
		--pill-fs: 0.72rem;
		--pill-h: 1.375rem;
